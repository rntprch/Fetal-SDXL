...
class SyntheticFetalDataset(Dataset):
    def __init__(self, root_dir, mapping, transform=None):
        self.root_dir = Path(root_dir)
        self.transform = transform
        self.class_to_idx = mapping
        
        self.folder_to_csv_name = {
            'trans_cerebellum': 'Trans-cerebellum',
            'trans_thalamic': 'Trans-thalamic',
            'trans_ventricular': 'Trans-ventricular'
        }
        
        self.samples = []
        for folder_name, csv_class_name in self.folder_to_csv_name.items():
            cls_folder = self.root_dir / folder_name
            if cls_folder.exists() and csv_class_name in self.class_to_idx:
                target_idx = self.class_to_idx[csv_class_name]
                if csv_class_name != "trans_thalamic":
                    for img_path in cls_folder.glob("*.png"):
                        self.samples.append((img_path, target_idx))
            else:
                print(f"Warning: Folder {folder_name} or class {csv_class_name} not found.")
...
def main():
    ...
    synth_ds = SyntheticFetalDataset(
        root_dir=Path("./SyntImages"), 
        mapping=full_dataset.class_to_idx,
        transform=transform
    )
    
    synth2_ds = SyntheticFetalDataset(
        root_dir=Path("./SyntImages2"), 
        mapping=full_dataset.class_to_idx,
        transform=transform
    )
    ...
    train_ds_plus_synth = ConcatDataset([train_ds, synth_ds, synth2_ds])
    
    train_loader = DataLoader(
        train_ds_plus_synth, batch_size=args.batch_size, shuffle=True)
